...
extension MainScreenCoordinator: MainScreenModuleOutput {
  ...
  func shareButtonAction(_ data: [(data: Data, name: String, extension: String)]) {
    ...
    DispatchQueue.global(qos: .userInteractive).async { [weak self] in
      ...
      data.forEach {
        if let imageFilePath = self?.services.fileManagerService.saveObjectWith(
          fileName: $0.name,
          ...
          self?.services.notificationService.showNegativeAlertWith(title: Appearance().somethingWentWrong,
                                                                   glyph: true,
                                                                   timeout: nil,
                                                                   active: {})
          dispatchGroup.leave()
