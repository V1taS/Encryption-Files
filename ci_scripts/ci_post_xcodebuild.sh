...
if [ -z "$CI_PULL_REQUEST_NUMBER" ] && [ -z "$CI_PULL_REQUEST_SOURCE_BRANCH" ]; then
    ...
    git remote set-url origin https://x-access-token:$GITHUB_TOKEN@github.com/V1taS/Encryption-Files.git
    
    # Пуш тегов
    git push --tags

    # Отправка уведомления в Telegram для мастер ветки
    python3 /Volumes/workspace/repository/Scripts/send_telegram.py "🔑🔑🔑 Релиз Encryption-Files iOS $MARKETING_VERSION 🔑🔑🔑
Сборка №: $CI_BUILD_NUMBER
URL сборки: $CI_BUILD_URL
Продукт: $CI_PRODUCT
Версия: $MARKETING_VERSION/$CURRENT_PROJECT_VERSION
Проект: https://github.com/V1taS/Encryption-Files
...
"
    ...
    python3 /Volumes/workspace/repository/Scripts/send_telegram.py "🔑 Новый Pull Request для Encryption-Files! 🔑
Сборка №: $CI_BUILD_NUMBER
URL сборки: $CI_BUILD_URL
Продукт: $CI_PRODUCT
Проект: https://github.com/V1taS/Encryption-Files
